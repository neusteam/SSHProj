...
public class MovieinfoDAO {
	...
	public List findByKey(String key) {
		...
		try {
			String queryString = "from Movieinfo as model where model.moviename like :key order by average desc";
			Query queryObject = getCurrentSession().createQuery(queryString);
	...
	public List findAverage(int count) {
		log.debug("finding all Movieinfo instances order by average");
		try {
			int m=0;
			int n=m+count;
			String queryString = "from Movieinfo order by average desc";
			Query queryObject = getCurrentSession().createQuery(queryString);
			queryObject.setFirstResult(m);
			queryObject.setMaxResults(n);
			return queryObject.list();
	...
	public List findDate(int count) {
		log.debug("finding all Movieinfo instances order by date");
		try {
			int m=0;
			int n=m+count;
			String queryString = "from Movieinfo order by date desc";
			Query queryObject = getCurrentSession().createQuery(queryString);
			queryObject.setFirstResult(m);
			queryObject.setMaxResults(n);
			return queryObject.list();
